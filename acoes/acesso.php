<?php
    ...
    Class SendMail{
        ...
        public function settingsMail($usuarioEmail, $senhaEmail){
            ...
            $this->mail->Username = "ispgayateste@gmail.com";
            $this->mail->Password = "ispgaya12@";
            $this->mail->SingleTo = true;

            $this->mail->From = "ispgayateste@gmail.com";
    ...
    Class Acesso extends SendMail{
        ...
        public function login($email, $senha){
            ...
            $query = $conexao->prepare("SELECT confirmado, senha, adm, ativo,nome, id FROM usuarios WHERE email = ?");
            $query->execute(array($email));

            if($query->rowCount()){
                $user = $query->fetchAll(PDO::FETCH_ASSOC)[0];

                if($user["senha"] == md5($senha) && $user["confirmado"] && $user["ativo"]){
                    session_start();
                    $_SESSION["usuario"] = array($user["nome"], $user["adm"], $user["id"], $user["ativo"]);
